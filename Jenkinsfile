    ...
    stages {
        ...
        stage('Deploy spring boot app') {
            steps {
                ...
                sh '''
                    ...
                    export ANSIBLE_CONFIG=~/workspace/ansible/ansible.cfg
                    ansible-playbook -i ~/workspace/ansible/hosts.yaml -l backend-vm-01 -e db_server_ip=35.246.17.240 -e user=jenkins -e group=jenkins ~/workspace/ansible/playbooks/spring.yaml
                '''
            }
        }
       stage('Deploy frontend') {
            steps {
                sh '''
                    export ANSIBLE_CONFIG=~/workspace/ansible/ansible.cfg
                    ansible-playbook -i ~/workspace/ansible/hosts.yaml -l frontend-vm-01 -e backend_server_ip=35.246.120.81 -e frontend_server_ip=34.147.199.240 ~/workspace/ansible/playbooks/vuejs.yaml
                '''
            }
       }
    }
